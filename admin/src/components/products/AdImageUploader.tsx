...
interface AdImageUploaderProps {
  initialExistingImages: ExistingAdImage[];
  onSelectionChange: (selection: AdImageSelection) => void;
}

function buildInitialSlots(existingImages: ExistingAdImage[]): ImageSlot[] {
  const slots: ImageSlot[] = new Array(MAX_AD_IMAGES)
    .fill(null)
    .map(() => ({ kind: "empty" }));
  existingImages.slice(0, MAX_AD_IMAGES).forEach((image, index) => {
    slots[index] = {
      kind: "existing",
      path: image.path,
      url: image.url,
...
export default function AdImageUploader({
  ...
  const [slots, setSlots] = useState<ImageSlot[]>(() =>
    buildInitialSlots(initialExistingImages),
  );
  ...
  useEffect(() => {
    setSlots((previousSlots) => {
      previousSlots.forEach((slot) => revokeSlotPreview(slot));
      return buildInitialSlots(initialExistingImages);
    });
  }, [initialExistingImages]);
  ...
  useEffect(() => {
    const retainedImagePaths = slots
    ...
    onSelectionChange({
      retainedImagePaths,
      newFiles,
      filledSlotsCount: retainedImagePaths.length + newFiles.length,
    });
  ...
  const previewImages = useMemo(
    () =>
      slots
        .filter((slot): slot is Extract<ImageSlot, { kind: "existing" | "new" }> => slot.kind !== "empty")
        .map((slot) => ({
          url: slot.kind === "existing" ? slot.url : slot.previewUrl,
          label: slot.kind === "existing" ? "Existing image" : slot.file.name,
        })),
    [slots],
  );
      ...
      >
        <ModalContent>
          <ModalHeader className="flex items-center gap-2">
            <ImagesSquare className="h-5 w-5 text-primary" />
